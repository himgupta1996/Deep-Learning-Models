...
for batch_size in batch_sizes:
    ...
    sgd = optimizers.SGD(lr=learning_rate, nesterov=True)
    ...
    history_cnn = cnn.fit(train_images, train_labels, epochs=epoch, batch_size= batch_size, verbose=1, validation_data=(val_images, val_labels))
    ...
    axes[0].plot(np.arange(1,epoch+1),Accuracy_validation,label="batch_size=%s"%(batch_size))
    axes[1].plot(np.arange(1,epoch+1),Accuracy_training,label="batch_size=%s"%(batch_size))
...
fig.savefig('Experiment_cnn_batch_size_final.jpg')
